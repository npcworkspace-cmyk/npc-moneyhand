...
test("release build is reproducible, confined, installable and includes an npm README", {
  ...
  assert.deepEqual(built.manifest.suite, { name: "npc-moneyhand", version: "1.2.3" });
  assert.equal(built.manifest.packages.length, 1);
  assert.equal(built.manifest.packages[0].package, "npc-moneyhand");
  assert.equal(built.manifest.packages[0].version, "1.2.3");
  assert.equal(built.manifest.extension.version, "1.2.3");
  assert.equal(built.manifest.extension.versionName, "1.2.3");
  assert.equal(
